  ...
  return (
    <div className="flex aspect-video w-fit flex-col gap-y-4 self-start rounded-xl bg-neutral-200/30 p-6 dark:bg-neutral-800/30">
      <div className="flex flex-col gap-y-1">
        {title && (
          <h3 className="text-xs font-semibold text-neutral-500 before:empty:inline-block">
            {title}
          </h3>
        )}
        <h4 className="text-2xl font-medium text-neutral-900 dark:text-neutral-300">{data}</h4>
      </div>
      {children}
    </div>
  );
    ...
    <>
      <div className="flex h-full w-full select-none flex-col items-center justify-center gap-8 p-12 font-medium tabular-nums md:p-0">
        <div className="flex w-full items-center justify-center whitespace-nowrap leading-none">
          <div className="flex">
            <div className="text-neutral-900 dark:text-orange-300" style={{ fontSize: "12vw" }}>
              {timeStr}
            </div>
      ...
      {Array.from({ length: total }).map((_, i) => (
        <div
          ...
          className={cn({
            ...
            "bg-orange-600/10 dark:bg-orange-900/70": current > i + 1,
            "bg-neutral-200 dark:bg-neutral-900": current < i + 1,
            "bg-orange-500 dark:bg-orange-500": current === i + 1,
          })}
        />
      ))}
    </>
